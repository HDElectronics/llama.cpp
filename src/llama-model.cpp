...
void llama_model::load_hparams(llama_model_loader & ml) {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_FALCON3VL:
            {
                ml.get_key(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS, hparams.f_norm_rms_eps);
                switch (hparams.n_layer) {
                    case 18: type = LLM_TYPE_1B;  break;
                    case 22: type = LLM_TYPE_3B;  break;
                    case 28: type = LLM_TYPE_7B;  break;
                    case 40: type = LLM_TYPE_10B; break;
                    default: type = LLM_TYPE_UNKNOWN;
                }
            } break;
...
struct llm_build_falcon3vl : public llm_graph_context {
    llm_build_falcon3vl(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
ggml_cgraph * llama_model::build_graph(const llm_graph_params & params) const {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_FALCON3VL:
            {
                llm = std::make_unique<llm_build_falcon3vl>(*this, params);
            } break;
        case LLM_ARCH_LFM2:
            {
